...
// Parse clean mode flag (delete existing TV Show children and exit)
bool cleanMode = args.Any(a => string.Equals(a, "--clean", StringComparison.OrdinalIgnoreCase) || string.Equals(a, "-c", StringComparison.OrdinalIgnoreCase));

// Maximize HTTP throughput for bulk operations by relaxing legacy defaults
ServicePointManager.DefaultConnectionLimit =1000;
ServicePointManager.Expect100Continue = false;
ServicePointManager.UseNagleAlgorithm = false;

// Local function encapsulating clean logic to avoid identifier collisions in top-level scope
async Task RunCleanAsync()
{
 Console.WriteLine("🧹 Clean mode: deleting all TV Show children under the configured parent...\n");
 ...
 var index = await heartcoreClient.GetChildrenIndexAsync(options.ShowsParentKey, CancellationToken.None);
 var keys = index.Values.Select(v => v.key).ToList();
 Console.WriteLine($"Found {keys.Count} items to delete\n");

 // Token bucket respecting HEARTCORE_RPS
 ...
 lock (bucketLock)
 {
 if (lastRefill.Elapsed >= TimeSpan.FromSeconds(1))
 {
 bucket = rps;
 lastRefill.Restart();
 }
 ...
 await Parallel.ForEachAsync(keys, new ParallelOptions { MaxDegreeOfParallelism = options.MaxDegreeOfParallelism }, async (key, ct) =>
 {
 try
 {
 ConsumeToken();
 await heartcoreClient.DeleteContentAsync(key, ct);
 ...
 var total = deleted + failed;
 if (total %500 ==0)
 {
 Console.WriteLine($"Progress: {total} deletions processed ({deleted} deleted, {failed} failed)");
 }
 });

 timer.Stop();
 Console.WriteLine($"\n✅ Clean complete: {deleted} deleted, {failed} failed in {timer.Elapsed:mm\\:ss}");
}
...
// Optimized HTTP client for TVMaze (HTTP/2 + decompression) to reduce payload size and latency
...
//1) Download all shows from TVMaze (streaming) page-by-page
while (true)
{
...
//2) Heartcore setup from environment variables (see README and HeartcoreOptions)
var options = HeartcoreOptions.FromEnvironment();
if (string.IsNullOrWhiteSpace(options.ProjectAlias) || string.IsNullOrWhiteSpace(options.ApiKey) || options.ShowsParentKey == Guid.Empty)
{
 Console.WriteLine("❌ Missing required Heartcore configuration.");
...
// Optimized HTTP client for Heartcore (HTTP/2 + pooling). Separate clients for management and media avoid HOL blocking.
var managementHttp = new HttpClient(new SocketsHttpHandler
{
 AutomaticDecompression = DecompressionMethods.GZip | DecompressionMethods.Deflate | DecompressionMethods.Brotli,
 PooledConnectionLifetime = TimeSpan.FromMinutes(10),
...
// Connectivity check to fail fast if configuration is wrong
try
{
 await heartcoreClient.ValidateParentAsync(options.ShowsParentKey, CancellationToken.None);
 Console.WriteLine("✅ Heartcore connectivity OK\n");
...
//3) Build index of existing children to avoid duplicates (idempotent migration)
...
// Lightweight token-bucket rate limiter honoring HEARTCORE_RPS to avoid hitting plan limits
var rps = Math.Max(1, options.RequestsPerSecond);
var bucket = rps; // tokens available in this second
var lastRefill = Stopwatch.StartNew();
var bucketLock = new object();
...
//4) Process shows in ascending id order for deterministic progress; optionally cap count with HEARTCORE_TAKE
...
//5) Optional batch publish in parallel per culture
